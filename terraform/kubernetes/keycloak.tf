...
resource "random_password" "keycloak_keystore_password" {
  length  = 16
  special = false
}

resource "random_password" "keycloak_truststore_password" {
  length  = 16
  special = false
}
...
resource "helm_release" "keycloak" {
  ...
  values = [
    ...
    tls:
      enabled: true
      keystorePassword: "${random_password.keycloak_keystore_password.result}"
      truststorePassword: "${random_password.keycloak_truststore_password.result}"
