...
echo
echo "────────────────────────────────────────────────────────"
echo "  GitLab setup — provide values when prompted"
echo "  (create PATs at: https://gitlab.com/-/user_settings/personal_access_tokens)"
echo "────────────────────────────────────────────────────────"
echo
read -rp "GitLab PAT with 'api' scope (to set CI/CD variables): " GITLAB_API_PAT
[[ -z "${GITLAB_API_PAT}" ]] && die "GitLab api PAT is required."

read -rp "GitLab PAT with 'read_repository' scope (for ArgoCD to pull repos): " GITLAB_REPO_PAT
[[ -z "${GITLAB_REPO_PAT}" ]] && die "GitLab read_repository PAT is required."

read -rp "GitLab username: " GITLAB_USERNAME
[[ -z "${GITLAB_USERNAME}" ]] && die "GitLab username is required."

read -rp "Let's Encrypt email [adriangherasim1@gmail.com]: " LETSENCRYPT_EMAIL
LETSENCRYPT_EMAIL="${LETSENCRYPT_EMAIL:-adriangherasim1@gmail.com}"

read -rp "Cloudflare API token (press Enter to skip): " CLOUDFLARE_TOKEN
read -rp "Cloudflare zone ID (press Enter to skip): " CLOUDFLARE_ZONE_ID
...
echo
echo "════════════════════════════════════════"
info "Step 2 — Creating S3 state bucket: ${STATE_BUCKET}"
echo "════════════════════════════════════════"
...
echo
echo "════════════════════════════════════════"
info "Step 3 — Resolving GitLab group and project IDs"
echo "════════════════════════════════════════"
...
echo
echo "════════════════════════════════════════"
info "Step 4 — Setting GitLab group-level CI/CD variables"
echo "════════════════════════════════════════"
...
# Deployment credentials for the new IAM user
...
echo
echo "════════════════════════════════════════"
info "Step 5 — Setting kubernetes-infrastructure project variables"
echo "════════════════════════════════════════"
...
echo
echo "════════════════════════════════════════"
info "Step 6 — Creating deployment pipeline trigger token"
echo "════════════════════════════════════════"
...
echo "  Access key ID:   ${DEPLOY_ACCESS_KEY_ID}"
